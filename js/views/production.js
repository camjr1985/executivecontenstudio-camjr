...
import { formatIcon, statusBadge, mediaBadge, emptyState, displayTitle } from '../components.js';
...
export function renderProduction(root, store, navigate) {
  ...
  const filters = [
    ...
    { key: 'media', label: 'Media pendente', test: r => r.media_status === 'PENDING_MEDIA' },
    { key: 'qc', label: 'QC pendente', test: r => !r.qc_status || r.qc_status === 'PENDING' },
    { key: 'owner', label: 'Aprovação do owner', test: r => r.owner_approval_required },
    { key: 'ready', label: 'Pronto', test: r => r.status === 'READY' || r.status === 'SCHEDULED' },
    { key: 'blocked', label: 'Bloqueado', test: r => r.owner_approval_required && (r.status === 'DRAFT' || r.status === 'PROPOSED') }
  ];
  ...
  function draw() {
    ...
    body.innerHTML = rows.length ? `<div class="table-wrap"><table><thead><tr>
      <th>Content ID</th><th>Data</th><th>Formato</th><th>Título</th><th>Readiness</th><th>Media</th><th>QC</th><th>Aprovação</th><th>Publicação</th>
      </tr></thead><tbody>${rows.map(r => `<tr data-open="${esc(r.content_id)}">
        ...
        <td>${mediaBadge(r.media_status)}</td>
        <td>${r.qc_status ? esc(r.qc_status) : '<span class="badge warn">PENDING</span>'}</td>
        <td>${r.owner_approval_required ? '<span class="badge danger">Owner gate</span>' : '<span class="badge ok">Não requerida</span>'}</td>
        <td><span class="badge">${esc(r.publication_status || 'PENDING')}</span></td>
      </tr>`).join('')}</tbody></table></div>` : emptyState('Nenhum item nesta categoria.');
